...
import { PLACEHOLDER_AP_REGEN_INTERVAL_DAYS, TICKS_PER_DAY } from '../data/balance';
...
export const placeholderHeroAbilityImpl: HeroAbilityImpl = {
  ...
  isMilestoneUnlocked: (state) => state.hero.milestoneUnlocked, // reads from hero state
};
...
export function processHero(
  ...
  // Use hero.maxActionPoints as the AP cap — allows class or guild bonuses to affect the cap
  const newAP = Math.min(Math.max(0, state.hero.actionPoints + regen), state.hero.maxActionPoints);
