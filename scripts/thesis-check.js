...
const checkLet = (line, lineNum) => {
  const noComment = line.replace(/\/\/.*$/, '')          // strip line comment
  const noStrings = noComment.replace(/'[^']*'|"[^"]*"|`[^`]*`/g, '""') // strip string literals
  const match     = noStrings.match(/\blet\s+/)
...
const checkClass = (line, lineNum) => {
  // Strip line comment, then strip string literals to avoid false positives
  const noComment = line.replace(/\/\/.*$/, '')
  const noStrings = noComment.replace(/'[^']*'|"[^"]*"|`[^`]*`/g, '""')
  const match     = noStrings.match(/(?:^|[;{}\s])(?:export\s+)?(?:abstract\s+)?class\s+\w/)
  if (!match) return null
  return {
    lineNum,
    col:  (match.index ?? 0) + 1,
...
const collectFiles = (dir) => {
  ...
  for (const entry of readdirSync(dir)) {
    ...
    if (stat.isDirectory()) {
      ...
      if (!STRICT && full.endsWith('.cy.ts'))   continue
      results.push(full)
    }
  }
  return results
