...
describe('adr_lookup', () => {
  it('lists all ADRs when no query provided', async () => {
    ...
    expect(parsed.adrs[0].number).toBe('001');
  });
  ...
  it('finds ADR by number', async () => {
    ...
    expect(parsed.results[0].content).toContain('pure function');
  });
  ...
  it('finds ADR by zero-padded number', async () => {
    ...
    const parsed = JSON.parse(result.content[0]?.text ?? '{}');
    expect(parsed.count).toBe(1);
    expect(parsed.results[0].number).toBe('001');
  });
  ...
  it('finds ADR by keyword', async () => {
    ...
    expect(parsed.results[0].title).toContain('mmkv');
  });
